...
def compare_all_variants(datasets):
    ...
    for dataset_name, dataset_info in datasets.items():
        ...
        for model_name, model in models.items():
            try:
                result = train_and_evaluate_model(X_train, X_test, y_train, y_test, model_name, model)
...
def visualize_comparison_results(results):
    ...
    pivot_acc = df_results.pivot(index='dataset', columns='model_name', values='accuracy')
    ...
    ax2.set_title('Cross-Validation Scores by Dataset Variant', fontweight='bold', fontsize=12)
    ...
    bars3 = ax3.bar(range(len(best_acc_per_dataset)), best_acc_per_dataset.values, 
                    color=['lightblue', 'lightcoral', 'lightgreen', 'lightyellow'][:len(best_acc_per_dataset)])
    ...
    bars4 = ax4.bar(range(len(model_avg_acc)), model_avg_acc.values, 
                    color=['skyblue', 'lightcoral', 'lightgreen', 'orange'][:len(model_avg_acc)])
    ax4.set_title('Average Model Performance Across All Datasets', fontweight='bold', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig('code/classification/clustering_comparison_results.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def generate_summary_report(results, importance_results):
    ...
    print(f"\n📊 DATASET COMPARISON:")
    dataset_performance = df_results.groupby('dataset').agg({
        'accuracy': ['mean', 'max'],
        'cv_mean': ['mean', 'max']
    }).round(4)
    ...
    for clustering_method in ['kmeans', 'hierarchical', 'dbscan']:
        ...
        if dataset_name in df_results['dataset'].values:
            enhanced_max = df_results[df_results['dataset'] == dataset_name]['accuracy'].max()
            improvement = enhanced_max - original_max
            improvement_pct = (improvement / original_max) * 100
            
            print(f"{clustering_method.upper()} clustering best accuracy: {enhanced_max:.4f} "
    ...
    for dataset_name, importance_dict in importance_results.items():
        if 'cluster' in str(importance_dict.keys()).lower():
            cluster_features = [f for f in importance_dict.keys() if 'cluster' in f.lower()]
            if cluster_features:
                cluster_importance = importance_dict[cluster_features[0]]
                print(f"   {dataset_name}: Cluster feature importance = {cluster_importance:.4f}")
    
    # Save detailed results
    results_df = pd.DataFrame(results)
    results_df.to_csv('code/classification/detailed_comparison_results.csv', index=False)
    
    # Create summary table
    summary_table = df_results.groupby(['dataset', 'model_name']).agg({
        'accuracy': 'first',
        'cv_mean': 'first',
        'cv_std': 'first',
        'n_features': 'first'
...
def main():
    ...
    print(f"\n✅ ANALYSIS COMPLETE!")
    print(f"Check the 'code/classification/' directory for all results and visualizations.")
